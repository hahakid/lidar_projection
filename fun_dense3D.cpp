...
int grid=15; // Size of the mask/kernel
int kin=0;
double mr = 0;
...
/*******************/ 
for (int v=0; v<dim[1]; v=v+1)// v<301
{
 for (int u=0; u<dim[2]; u=u+1)// u<1242
 {
    S=0; Y=0;
        ...
        )
{
    ...
    /* Check for proper number of arguments */
        ...
        mexErrMsgIdAndTxt("MATLAB:mexcpp:nargin",//
                "MEXCPP requires 2 input arguments.");// Ima3D(c_px:end,:) = fun_dense3D(Pts,[c_px i_size]);
    } else if (nlhs >= 2) {
        mexErrMsgIdAndTxt("MATLAB:mexcpp:nargout",//
                "MEXCPP requires 1 output argument.");// Ima3D(c_px:end,:) = fun_dense3D(Pts,[c_px i_size]);
    } 
